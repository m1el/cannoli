...
The safety results (1 and 2) also hold for **every RC11-consistent
execution**, through a mechanized RC11 ⇒ ORC11 correspondence (see below), so
the trust base for them is the RC11 axioms rather than the ORC11 machine.
...
`ORC11/RC11.lean` defines RC11 execution graphs of our thread pools, following
the ORC11 appendix §2.1 (Lahav et al.'s RC11 without SC accesses and fences):
labels, `sb`, `rf`, `mo`, `rb`, `eco`, release sequences, `sw`, `hb`,
consistency (well-formedness, COHERENCE, ATOMICITY, NO-THIN-AIR) and races.
Representation choices, all standard:

- Events are numbered in an order extending `sb ∪ rf`. Such a numbering exists
  exactly when NO-THIN-AIR holds.
- The initialization writes are implicit events, `sb`-before everything.
- `mo` is given by dense ranks per location.
- ATOMICITY is stated for single-event updates: an update's source is its
  immediate `mo`-predecessor. The appendix lists no atomicity axiom, which for
  single-event updates would let an update read a write `mo`-after itself.
- Each thread's events are generated by running its program, choices included.

`ORC11/Replay.lean` proves the correspondence:

```lean
theorem RC11.Exec.replay (hc : G.Consistent) (hd : Disciplined prog A) :
    (∃ c, Reachable prog s0 v0 c ∧ Racy prog c) ∨
      (¬ G.Racy ∧ ∃ c, Reachable prog s0 v0 c ∧ ∀ π, (c.th π).1 = G.final π)
```

Every RC11-consistent graph `G` of a *location-disciplined* pool (each
location accessed only atomically, with relaxed or acquire/release orders, or
only non-atomically) either leads ORC11 to a race, or is race-free and is an
ORC11 run ending in the same thread-local states. The proof replays `G` in its
numbering. Its invariant keeps upper bounds on thread and message views
(every time is that of a write the thread has seen through `rf? ; hb*`,
message views are bounded by their release-sequence heads) and lower bounds on
the race detector. At the first event that would complete an RC11 race,
ORC11's race check fails (`hidden_write`, `raceFree_succ`). This is the
appendix's Theorem 1 and Lemmas 3–8, proved directly on ORC11 instead of
through its operational graph semantics.

The discipline hypothesis is needed. With a location accessed both ways, the
Coq ORC11 lets an acquire load of a *relaxed* store count that store as seen
...
theorem Mempipe.rc11_safe (G : RC11.Exec (prog N M pay ln) init0 initVal) (hc : G.Consistent) :
    ¬ G.Racy ∧ (G.final .sender).pc ≠ .fault ∧ (∀ r, (G.final (.recv r)).pc ≠ .fault) ∧
      ...
      (∀ r r', r ≠ r' → ∀ t ∈ (G.final (.recv r)).log.map Prod.fst,
        t ∉ (G.final (.recv r')).log.map Prod.fst)
```
...
| `Mempipe/RC11.lean` | mempipe is disciplined; safety under RC11 |
